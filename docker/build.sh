...
docker build --no-cache=true --build-arg BUILD_DATE=$(date -u +%Y-%m-%dT%H:%M:%S) --build-arg VCS_REF=${git_hash} --build-arg BUILD_VERSION=${version} -t pdssp/planetary-crs-registry .
