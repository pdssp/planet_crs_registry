...
def test_version_2015(conn):
    ...
    try:
        response = requests.get("http://localhost:8080/ws/versions/2015?limit=1&offset=0")
        response.raise_for_status()  # Raise an HTTPError for bad responses
        content = json.loads(response.text)
        assert content[0]["id"] == json_response[0]["id"]
        assert len(content) == 1
...
def test_solar_bodies_mars(conn):
    ...
    try:
        response = requests.get("http://localhost:8080/ws/solar_bodies/mars?limit=1&offset=0")
        response.raise_for_status()  # Raise an HTTPError for bad responses
        content = json.loads(response.text)
...
def test_search_mars(conn):
    ...
    try:
        response = requests.get("http://localhost:8080/ws/search?search_term_kw=mars&limit=1&offset=0")
        response.raise_for_status()  # Raise an HTTPError for bad responses
        content = json.loads(response.text)
        assert content[0]["id"] == result_json[0]["id"]
    except requests.RequestException as e:
...
def test_search_mars_count(conn):
    try:
        response = requests.get("http://localhost:8080/ws/search/count?search_term_kw=mars")
        response.raise_for_status()  # Raise an HTTPError for bad responses
        content = int(response.text)
        assert content == 51
    except requests.RequestException as e:
